...
execute context x = case x of
   ...
   STry try catch finally -> executeTryCatch context x

executeTryCatch :: RContext -> Stm -> Either ErrorMessage RContext
executeTryCatch context (STry [] catch finally) = execute context (SBlock finally)
executeTryCatch context (STry (stm:stms) catch finally) = case result of
   Left _ -> execute context (SBlock (catch ++ finally))
   Right newcontext -> executeTryCatch newcontext (STry stms catch finally)
   where result = execute context stm
executeTryCatch context _ = Left "Error: Not a Try-Catch-Finally block."
